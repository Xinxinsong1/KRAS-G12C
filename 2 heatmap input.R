library(DESeq2)
mycounts<- read.table("Untreat.txt", header = TRUE, quote = '\t',skip =0)
sampleNames <- c("C1","C2","C3","R1","R2","R3")
names(mycounts)[3:8] <- sampleNames
head(mycounts)
...
#变成矩阵
countMatrix <- as.matrix(mycounts[2:7])
table2 <- data.frame(name = c("C1","C2","C3","R1","R2","R3"),
                     condition = c("C","C","C","R","R","R"))
...
#火山图top100
library('pheatmap')
select <- order(rowMeans(normalized_counts),decreasing=T)[1:100]
ntd <- normTransform(dds_norm) #default to log2FC+1
log2.norm.counts <- assay(ntd)[select,]
df <- as.data.frame(colData(dds_norm)[,c("name","condition")])
out<-pheatmap(log2.norm.counts, cluster_rows=T, show_rownames=T,
              cluster_cols=T, annotation_col=df,scale="row",
              treeheight_col=0,clustering_method = "average",
              color = colorRampPalette(c("navy", "white", "firebrick3"))(491),
              clustering_distance_rows = "maximum",cutree_cols = 2,
              main="2D treatment vs control top100")

#从这里开始为后面分析通路输出要用的表格
res <- results(dds_norm,contrast=c("condition","R","C"))
head(res)
...
head(res4_order)
write.csv(res4_order,file="res4_log_order_R_vs_C.csv") #差异表达基因且有SYMBOL
